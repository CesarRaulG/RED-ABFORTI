...
/////////////////////////////////////////////////////////////////////////	
	...
	// Complemento de Pago	
	$("#action_upload_comple").click(function(e) {
		e.preventDefault();
	    actionUploadComplemento();
	});
	...
	function actionLogin() {
		...
		if (errorCounter > 0) {
		    ...
		    $("#response").removeClass("alert-success").addClass("alert-warning").fadeIn();
		    $("#response .message").html("<strong>Error</strong>: Por favor llena todos los!");
		    $("html, body").animate({ scrollTop: $('#response').offset().top }, 1000);
	...
	function actionUploadComplemento() {
		var errorCounter = validateForm();
	
		if (errorCounter > 0) {
			// Limpiar mensajes previos en el modal
			$('#uploadModal .modal-body .alert').remove();
	
			// Mostrar el mensaje de error solo en el modal
			$('#uploadModal .modal-body').prepend(`
				<div class="alert alert-warning" role="alert">
					<strong>Error:</strong> Parece que has olvidado completar algo.
				</div>
			`);
		} else {
			$(".required").parent().removeClass("has-error");
	
			var formData = new FormData($("#upload_comple")[0]);
			var $btn = $("#action_upload_comple").button("loading");
	
			// Enviar datos al servidor usando AJAX
			$.ajax({
				url: '../response.php', // Asegúrate de que esta URL esté correcta
				type: 'POST',
				data: formData,
				dataType: 'json',
				processData: false,
				...
				success: function(data) {
					...
					if (data.status === 'success') {
						...
						setTimeout(function () {
							...
							window.location.href = 'invoice-list.php'; // Redirigir a invoice-list.php
						}, 1500);
					} else {
						// Mostrar mensaje de advertencia
						$('#uploadModal .modal-body').prepend(`
							<div class="alert alert-warning" role="alert">
								<strong>Advertencia:</strong> ${data.message}
							</div>
						`);
					}
				...
				error: function(data) {
					...
					$('#uploadModal .modal-body').prepend(`
						<div class="alert alert-danger" role="alert">
							<strong>Error:</strong> No se pudo procesar la solicitud. Intenta nuevamente.
						</div>
					`);
					$btn.button("reset");
				}
