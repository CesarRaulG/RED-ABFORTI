...
// Obtener el RFC del proveedor desde la base de datos
if ($mysqli->connect_error) {
    die('Error : (' . $mysqli->connect_error . ') ' . $mysqli->connect_error);
}

$query = "SELECT rfc FROM providers WHERE id_proveedor = ?";
$stmt = $mysqli->prepare($query);
$stmt->bind_param("i", $_SESSION['id_proveedor']);  // Asegúrate de tener la variable id_proveedor en la sesión
$stmt->execute();
$result = $stmt->get_result();
$rfcProveedor = $result->fetch_assoc()['rfc'];
$stmt->close();
$mysqli->close();

function consultarCFDI($rfc_emisor, $rfc_receptor_esperado, $total_facturado, $uuid_timbrado)
{
...
function validarXMLCargar($archivos, $iniciales, $rfcProveedor)
{
    $xmls = array();
    $rfc_receptor_esperado = "INM180529GAA"; // Reemplaza esto con el RFC del receptor que esperas
    $mensajesErrores = [];
    $erroresPorArchivo = [];
    ...
    foreach ($archivos['tmp_name'] as $key => $tmp_name) {
        ...
        */
                ...
                if ($rfc_emisor !== $rfcProveedor) {
                    $erroresPorArchivo[$nombreOriginal][] = 'El RFC del emisor no coincide con tu RFC.';
                    continue;
                }
...
// Llamar a la función para verificar el bloqueo de subida
verificarBloqueoSubida($mysqli, $rfcProveedor);


if (isset($_POST['Comprobar'])) {
    $resultados = validarXMLCargar($_FILES['archivos'], $iniciales, $rfcProveedor);
}
...
$paginaRedireccion = obtenerRedireccion($rol, 'status-fac.php', $validated, $no_recurrent);
                ...
                <tbody>
                    <?php foreach ($resultados as $key => $resultado): ?>
                        <tr id="fila-<?php echo $key; ?>">
                            ...
                            <td>
                                <form method="POST" enctype="multipart/form-data" class="form-guardar" data-id="<?php echo $key; ?>">
                                    ...
                                    <input type="hidden" name="rol" value="<?php echo $resultado['ROL']; ?>">
