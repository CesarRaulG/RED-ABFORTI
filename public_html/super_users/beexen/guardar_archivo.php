...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['archivo_temporal'])) {
   ...
   */
    ...
    // Directorio base
    $directorioBase = $_SERVER['DOCUMENT_ROOT'] . "/super_users/beexen/uploads/factura/NO_RECURRENTES/";
    ...
    if ($serie && $folio) {
        $nombreArchivoSF = $nombreEmisor . ' ' . $serie . ' ' . $folio;
    } else {
        $nombreArchivoSF = $nombreEmisor . ' ' . obtenerUltimosCuatroDigitos($UUID);
    }

    // Reemplazar caracteres especiales y espacios en el nombre del archivo
    $nombreArchivoSF = str_replace(' ', '_', $nombreArchivoSF);
    $nombrePermanente = $directorioPermanente . $nombreArchivoSF . '.' . $extension;

    $nombreArchivoFinal = $nombreArchivoSF . '.' . $extension;
    ...
    if ($result->num_rows > 0) {
        ...
        $cod_empresa = 'BE1';
            ...
            $cfdiData = array(
                ...
                'factura_xml' => $nombreArchivoFinal
            ...
            $stmtCFDI->bind_param(
                "ssssssdsssssssssssssssssssssss",
                $cfdiData['rfc_emisor'],
        ...
        if (rename($archivoTemporal, $nombrePermanente)) {
            ...
            $noRecurrent = 1; // Define el valor que deseas asignar a no_recurrent
